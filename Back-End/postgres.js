...
app.post('/add-rating/:username', async (req, res) => {
    const { username } = req.params;
    const { rating } = req.body;

    try {
        // Add the new rating to the database
        await client.query('INSERT INTO ratings (username, rating) VALUES ($1, $2)', [username, rating]);
        
        // Fetch the updated data and send it back to the client
        const updatedData = await handleData();
...
app.get('/average-rating/:username', async (req, res) => {
    const { username } = req.params;

    try {
        const result = await client.query('SELECT AVG(rating) AS avg_rating FROM ratings WHERE username = $1', [username]);
