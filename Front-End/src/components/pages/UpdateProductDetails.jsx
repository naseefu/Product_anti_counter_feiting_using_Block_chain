...
import { Button, TextField, Box, Paper, Typography, Autocomplete } from '@mui/material';
...
const UpdateProductDetails = () => {
    ...
    const [showRating, setShowRating] = useState(false); // State to show rating for retailers
    ...
    const getUsername = async () => {
        try {
            ...
            setCurrLocation(userData.location); // Assuming the location field is named 'location' in the backend
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await axios.post(`http://localhost:5000/add-rating/${currName}`, { rating: ratingValue });
    ...
    //         const res = await axios.post(`http://localhost:5000/add-rating/${currName}`, { rating: ratingValue });
    //         console.log(res.data);
    //         // You can add further logic here if needed, like updating state to reflect the new rating
    //     } catch (error) {
    //         console.error(error);
    //     }
    // };
            ...
            <Paper elevation={3} sx={{ width: "400px", margin: "auto", marginTop: "10%", marginBottom: "10%", padding: "3%", backgroundColor: "#e3eefc" }}>
                ...
                {auth.role === "supplier" ? null
                    : <React.Fragment>
                        ...
                        {auth.role === "retailer" && (
                            <div>
                            <Typography variant="body1">Rate the Retailer:</Typography>
                                <Rating
                                    name="rating"
                                    value={ratingValue}
